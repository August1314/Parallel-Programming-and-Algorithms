#ifndef LAB1_DOCKER_MATMUL_COMMON_HPP
#define LAB1_DOCKER_MATMUL_COMMON_HPP
...
inline MatrixRunConfig parse_args(int argc, char** argv) {
    if (argc < 4) {
        throw std::runtime_error("usage: <m> <n> <k> [seed]");
    }
    ...
    config.seed = argc >= 5 ? static_cast<std::uint64_t>(std::stoull(argv[4])) : 20250401ULL;
...
int run_program(const std::string& version, int argc, char** argv, Kernel kernel) {
    try {
        ...
        std::cout << "time_sec=" << elapsed.count() << '\n';
