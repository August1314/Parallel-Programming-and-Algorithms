which uv 2>/dev/null || (python3 -m pip install uv 2>/dev/null || pip install uv) && export PATH="$HOME/.local/bin:$PATH"
which nvcc 2>/dev/null || module load cuda 2>/dev/null || echo "请手动加载CUDA环境"
test -d ~/repo || git clone https://github.com/August1314/Parallel-Programming-and-Algorithms.git ~/repo
cd ~/repo && git pull && cd ~/repo/lab/lab11
mkdir -p bin results results/figures
rm -rf bin/*
chmod +x scripts/build.sh scripts/run_conv.sh
./scripts/build.sh
./scripts/build.sh cudnn
./scripts/run_conv.sh 32 1 1
./scripts/run_conv.sh 64 1 1 8 8
./scripts/run_conv.sh 32 1 2
./scripts/run_conv.sh 64 2 2
./scripts/run_conv.sh 32 1 3
./scripts/run_conv.sh 64 3 3
test -f bin/conv_cudnn && ./scripts/run_conv.sh 32 1 4
python3 -m uv run python3 -c "import matplotlib; import numpy" 2>/dev/null || python3 -m uv pip install matplotlib numpy
python3 -m uv run python scripts/benchmark.py
python3 -m uv run python scripts/plot.py
ls results/figures/ | head
cd ~
tar -czf lab11_results.tar.gz repo/lab/lab11/results/ repo/lab/lab11/report/
ls -lh lab11_results.tar.gz
